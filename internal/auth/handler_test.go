...
func setupTestRouter() *gin.Engine {
	...
	// Create a simple handler that just validates JSON
	router.POST("/auth/login", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"message": "login would be processed"})
	})
	...
	router.POST("/auth/register", func(c *gin.Context) {
		...
		c.JSON(http.StatusCreated, gin.H{"message": "register would be processed"})
	})
...
func TestHandler_Register_InvalidJSON(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/auth/register", bytes.NewBuffer([]byte("invalid json")))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
